...
export function CloudflareR2Form({ provider }: { provider: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      else toast.success("Cloudflare R2 config saved successfully")
    } catch (err: any) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-slate-900 border rounded-xl p-6">
      <h3 className="text-lg font-semibold border-b pb-4">Cloudflare R2 Settings</h3>
      ...
      <div className="space-y-4">
        <div>
          ...
          <input 
            ...
            placeholder="e.g. 0a1b2c3d4e5f6g7h8i9j" 
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input 
              ...
              type="password" 
              className="w-full border rounded-lg px-3 py-2 bg-slate-50 dark:bg-slate-800" 
              placeholder={provider._hasSecret ? "•••••••••••••••• (Set)" : "Enter Secret Access Key"} 
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input 
              ...
              className="w-full border rounded-lg px-3 py-2 bg-slate-50 dark:bg-slate-800" 
              placeholder="e.g. tao-video-assets" 
            />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Public URL Prefix (Optional)</label>
            <input 
              name="publicUrl"
              type="text" 
              defaultValue={config.publicUrl || ""}
              className="w-full border rounded-lg px-3 py-2 bg-slate-50 dark:bg-slate-800" 
              placeholder="e.g. https://cdn.domain.com" 
            />
          </div>
        </div>
        ...
        <div className="flex gap-6 pt-4 border-t">
          ...
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="is_default" className="text-sm font-medium">Set as Default Storage</label>
          </div>
        </div>
      </div>
